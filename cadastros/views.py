...
def cadastrar_militar(request):
    if request.method == 'POST':
        ...
        if numero == '' or namebm == '' or postgrad == '' or localbm == '':
            messages.add_message(request, messages.ERROR, 'Os campos não podem estar vazios!')
        ...
        if Militar.objects.filter(numero=numero).exists():
            messages.add_message(request, messages.ERROR, 'Militar já cadastrado!')
            return redirect(reverse('cadastrar_militar'))
        ...
        militar = Militar.objects.create(numero=numero, nome=namebm, postgrad=postgrad, local=localbm, empenhos_minutos_atipica=empenhos_minutos_atipica, qtd_empenhos=qtd_empenhos, empenhos_minutos_tipica=empenhos_minutos_tipica)
        ...
        messages.add_message(request, messages.SUCCESS, 'Militar cadastrado com sucesso!')
        return redirect(reverse('cadastrar_militar'))
    else:
...
def cadastrar_empenho(request):
    ...
    if request.method == 'POST':
        ...
        datahorainicial = datetime.strptime(datahorainicial_str, '%d/%m/%Y %H:%M')
        ...
        if Empenho.objects.filter(nome=namemp).exists():
            messages.add_message(request, messages.ERROR, 'Empenho já cadastrado!')
            return redirect(reverse('cadastrar_empenho'))
        ...
        empenho = Empenho.objects.create(nome=namemp, tipo=tipoemp, datahorainicial=datahorainicial, datahorafinal=datahorafinal, duracao_em_minutos_atipica=horatip, duracao_em_minutos_tipica=hortip, documentacao=hiddensei, observacoes=obsemp)
            ...
            if militar:
                ...
                if Militar.objects.filter(numero=militar).exists():
                    ...
                    if not militar_obj.empenhos.filter(nome=namemp).exists():
                        ...
                        militar_obj.qtd_empenhos = int(militar_obj.qtd_empenhos) + 1
                        militar_obj.empenhos_minutos_tipica = int( militar_obj.empenhos_minutos_tipica) + int(hortip)
                        militar_obj.empenhos_minutos_atipica = int( militar_obj.empenhos_minutos_atipica) + int(horatip)
                        militar_obj.save()
                else:
...
def editar_empenho(request, id_empenho):
    if request.method == 'POST':
        ...
        horatip = request.POST.get('horatip', '')
        all_militares = request.POST.get('emp_mil_data', '')
        all_militares_conv = [mil.strip() for mil in all_militares.split(',')] # Vem na lista o numero de BM do militar
        ...
        messages.add_message(request, messages.SUCCESS, 'Empenho editado com sucesso!')
    ...
    if request.method == 'POST':
        ...
        if empenho.militares.all():
            ...
            for militar in militares:
                ...
                militar.empenhos_minutos_atipica = int(militar.empenhos_minutos_atipica) - empenho.duracao_em_minutos_atipica
                militar.empenhos_minutos_tipica = int(militar.empenhos_minutos_tipica) - empenho.duracao_em_minutos_tipica
                militar.save()
        try:
            empenho.delete()
            messages.add_message(request, messages.SUCCESS,
...
def edit_militar(request, id_militar):
    if request.method == 'POST':
        ...
        # Salva as alterações no banco de dados
        militar.save()
        messages.add_message(request, messages.SUCCESS, 'Militar editado com sucesso!')
        return redirect(reverse('registrar_empenho'))
    else:
        return HttpResponse("Você não tem autorização para acessar essa pagina", status=401)
...
def is_permited_empenho(request, militar_id, empenho_id):
    ...
    if indisponibilidades:
        ...
        for indisponibilidade in indisponibilidades:
            ...
            is_permited = verificar_sobreposicao(data_hora_inicial_empenho, data_hora_final_empenho, data_hora_inicial_indisponibilidade, data_hora_final_indisponibilidade)
            tipo = indisponibilidade.tipo
            descricao = indisponibilidade.descricao
            obj_indisp = [tipo, descricao, data_hora_inicial_indisponibilidade, data_hora_final_indisponibilidade]
            if not is_permited:
    ...
    if emp_militar:
        ...
        for emp_militar in empenhos_do_militar:
            ...
            is_permited = verificar_sobreposicao(data_hora_inicial_empenho, data_hora_final_empenho, datah_init, datah_end)
...
def get_mil_data(request, militar_id):
    militar = Militar.objects.get(id=militar_id)
    empenhos = list(militar.empenhos.values())  # Convert QuerySet to a list of dicts
    indisponibilidades = list(IndispMilitar.objects.filter(militar_id=militar_id))  # Convert QuerySet to a list of dicts
    ...
    if empenhos:
        ...
        for empenho in empenhos:
            ...
            datahorainicial = datetime.strptime(datahorainicial.strftime('%d/%m/%Y %H:%M'), '%d/%m/%Y %H:%M')
            datahorafinal = datetime.strptime(datahorafinal.strftime('%d/%m/%Y %H:%M'), '%d/%m/%Y %H:%M')
            ...
            if datainicial <= datahorainicial and datafinal >= datahorafinal:
